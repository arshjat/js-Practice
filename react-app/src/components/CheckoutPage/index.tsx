...
export default function CheckoutPage():React.ReactElement {
  ...
  const bagTotal = useMemo(() => itemList.reduce((sum:number, item:[string, number]) => sum + item[1] * (database.get(item[0]).price.replace(',', '')), 0), [itemList]);
        ...
        <section className="left-pane">
          <section className="header-leftpane">
            <p>
              My shopping Bag
              (
              {itemList.reduce((sum:number, item:[string, number]) => sum + item[1], 0)}
              items
              )
            </p>
          </section>
